...
/* BTU internal for BR/EDR and AMP HCI command timeout (reserve up to 3 AMP
 * controller) */
 ...
 */
#if (defined(NFC_SHARED_TRANSPORT_ENABLED) &&                                  \
     (NFC_SHARED_TRANSPORT_ENABLED == TRUE))
#define BTU_NFC_AVAILABLE_EVT                                                  \
    EVENT_MASK(APPL_EVT_0) /* Notifies BTU task that NFC is available (used    \
                              for shared NFC+BT transport) */
#endif
...
typedef struct {
    ...
    tBTU_EVENT_CALLBACK
        event_cb; /* callback triggered when event is in range */
} tBTU_EVENT_REG;
...
#if (defined(BTU_CMD_CMPL_TOUT_DOUBLE_CHECK) &&                                \
     BTU_CMD_CMPL_TOUT_DOUBLE_CHECK == TRUE)
...
typedef struct {
    ...
    TIMER_LIST_Q
        quick_timer_queue;    /* Timer queue for transport level (100/10 msec)*/
...
#if BLE_INCLUDED == TRUE
    UINT16
        hcit_ble_acl_data_size; /* Max BLE ACL data size across HCI transport */
    UINT16 hcit_ble_acl_pkt_size; /* Max BLE ACL packet size across HCI
                                     transport  */
    /* (this is data size plus 4 bytes overhead) */
#endif
...
************************************
*/
BTU_API extern void btu_start_timer(TIMER_LIST_ENT* p_tle, UINT16 type,
                                    UINT32 timeout);
BTU_API extern void btu_stop_timer(TIMER_LIST_ENT* p_tle);
BTU_API extern void btu_register_timer(TIMER_LIST_ENT* p_tle, UINT16 type,
                                       UINT32 timeout,
                                       tBTU_TIMER_CALLBACK timer_cb);
BTU_API extern void btu_deregister_timer(TIMER_LIST_ENT* p_tle);
//! This function is not in Wii Sports
// BTU_API extern UINT32 btu_remaining_time (TIMER_LIST_ENT *p_tle);

BTU_API extern void btu_register_event_range(UINT16 range,
...
BTU_API extern void btu_start_quick_timer(TIMER_LIST_ENT* p_tle, UINT16 type,
                                          UINT32 timeout);
