...
BOOL RFLiIsSameFaceCore(const RFLiCharInfo* lhs, const RFLiCharInfo* rhs) {
    if ((lhs->eye.rawdata & EYE_PAD_MASK) !=
        (rhs->eye.rawdata & EYE_PAD_MASK)) {
        return FALSE;
    }

    if ((lhs->eyebrow.rawdata & EYEBROW_PAD_MASK) !=
        (rhs->eyebrow.rawdata & EYEBROW_PAD_MASK)) {
        return FALSE;
    }

    if ((lhs->mouth.rawdata & MOUTH_PAD_MASK) !=
        (rhs->mouth.rawdata & MOUTH_PAD_MASK)) {
        return FALSE;
    }

    if ((lhs->beard.rawdata & BEARD_PAD_MASK) !=
        (rhs->beard.rawdata & BEARD_PAD_MASK)) {
        return FALSE;
    }

    // @bug Glasses have no padding bits (this will mask the Y-position LSB)
    // May be a remanant of an older design?
    if ((lhs->glass.rawdata & GLASS_PAD_MASK) !=
        (rhs->glass.rawdata & GLASS_PAD_MASK)) {
        return FALSE;
    }

    if ((lhs->mole.rawdata & MOLE_PAD_MASK) !=
        (rhs->mole.rawdata & MOLE_PAD_MASK)) {
        return FALSE;
    }

    if ((lhs->nose.rawdata & NOSE_PAD_MASK) !=
        (rhs->nose.rawdata & NOSE_PAD_MASK)) {
        return FALSE;
    }

    if ((lhs->hair.rawdata & HAIR_PAD_MASK) !=
        (rhs->hair.rawdata & HAIR_PAD_MASK)) {
        return FALSE;
    }

    if ((lhs->faceline.rawdata & FACELINE_PAD_MASK) !=
        (rhs->faceline.rawdata & FACELINE_PAD_MASK)) {
        return FALSE;
    }
