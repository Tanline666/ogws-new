...
RFLErrcode RFLiBootLoadDatabaseAsync(RFLiCallback callback) {
    ...
    switch (RFLiOpenAsync(RFLiFileType_Database, NAND_ACCESS_READ,
...
#pragma unused(ctx)
    ...
    while (count < size) {
        ...
        if (curCount >= ASYNC_CRC_STEP) {
            ...
            OSSetAlarm(&mgr->crcInfo.alarm, OS_MSEC_TO_TICKS(19),
                       alarmCreateCb_);
            return;
        }
...
void RFLiCreateHeaderCRCAsync(RFLiExCallback callback) {
    ...
    OSCancelAlarm(&mgr->crcInfo.alarm);
    OSSetAlarmUserData(&mgr->crcInfo.alarm, &mgr->crcInfo);
    OSSetAlarm(&mgr->crcInfo.alarm, OS_MSEC_TO_TICKS(19), alarmCreateCb_);
}
...
void RFLiCheckHeaderCRCAsync(RFLiExCallback callback) {
    ...
    OSCancelAlarm(&mgr->crcInfo.alarm);
    OSSetAlarmUserData(&mgr->crcInfo.alarm, &mgr->crcInfo);
    OSSetAlarm(&mgr->crcInfo.alarm, OS_MSEC_TO_TICKS(19), alarmCheckCb_);
}
