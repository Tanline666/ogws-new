...
BTU_API UINT32 btu_task(UINT32 param) {
     ...
     * bta_sys_init() reset the control blocks and preset the trace level with
        ...
        if (event & TASK_MBOX_0_EVT_MASK) {
            ...
            while ((p_msg = (BT_HDR*)GKI_read_mbox(BTU_HCI_RCV_MBOX)) != NULL) {
                ...
                case BT_EVT_TO_BTU_HCI_CMD:
                    btu_hcif_send_cmd((UINT8)(p_msg->event & BT_SUB_EVT_MASK),
                                      p_msg);
                    break;
                ...
                case BT_EVT_TO_BTU_SAP:
                    ...
                    break;
#endif /* SAP */
#if (defined(GAP_CONN_INCLUDED) && GAP_CONN_INCLUDED == TRUE &&                \
     GAP_CONN_POST_EVT_INCLUDED == TRUE)
        ...
        if (event & TIMER_0_EVT_MASK) {
            ...
            while ((btu_cb.timer_queue.p_first) &&
                   (!btu_cb.timer_queue.p_first->ticks)) {
                ...
                case BTU_TTYPE_BTU_CMD_CMPL:
                    btu_hcif_cmd_timeout(
                        (UINT8)(p_tle->event - BTU_TTYPE_BTU_CMD_CMPL));
                    break;
                ...
                case BTU_TTYPE_USER_FUNC: {
                    tUSER_TIMEOUT_FUNC* p_uf =
                        (tUSER_TIMEOUT_FUNC*)p_tle->param;
                    (*p_uf)(p_tle);
                } break;
...
#endif /* defined(QUICK_TIMER_TICKS_PER_SEC) && (QUICK_TIMER_TICKS_PER_SEC >   \
          0) */
...
void btu_check_bt_sleep(void) {
    if ((btu_cb.hci_cmd_cb[LOCAL_BR_EDR_CONTROLLER_ID].cmd_cmpl_q.count == 0) &&
        ...
        if (l2cb.controller_xmit_window == l2cb.num_lm_acl_bufs) {
            /* enable dev to sleep  in the cmd cplt and cmd status only and num
             * cplt packet */
